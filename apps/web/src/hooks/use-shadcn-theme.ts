...
/** Captures h, s, l from a bare shadcn HSL triplet for manual parsing. */
const HSL_TRIPLET_CAPTURE = /^(-?[\d.]+)\s+(-?[\d.]+)%\s+(-?[\d.]+)%/;
...
function parseColor(raw: string, target: Color): Color {
  ...
  const triplet = value.match(HSL_TRIPLET_CAPTURE);
  if (triplet) {
    const h = Number.parseFloat(triplet[1]) / 360;
    const s = Number.parseFloat(triplet[2]) / 100;
    const l = Number.parseFloat(triplet[3]) / 100;
