...
const AVATAR_THUMB_MAX_SIDE = 128;
const AVATAR_FULL_MAX_SIDE = 512;
...
export const optimizeUserAvatarImages = (
  source: Buffer
    ...
    try: async () => {
      const { full, placeholder, thumb } = await BunImage.batch(source, {
        ...
        pipelines: {
          full: (img) =>
            img
              .resize(AVATAR_FULL_MAX_SIDE, AVATAR_FULL_MAX_SIDE, {
                filter: "lanczos3",
                fit: "inside",
          ...
          thumb: (img) =>
            img
              .resize(AVATAR_THUMB_MAX_SIDE, AVATAR_THUMB_MAX_SIDE, {
                filter: "lanczos3",
                fit: "inside",
                withoutEnlargement: false,
              })
